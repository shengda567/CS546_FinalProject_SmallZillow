...
router.post('/', async (req, res) =>{
    ...
    try{
        let tempMana = await ManagersData.getManagerByUsername(managerInfo.username);
        if(tempMana){
...
router.patch('/:id', async (req, res) =>{
    ...
    try{
        ...
        if (requestBody.password){
            ...
            try{
                compareToMatch = await bcryptjs.compare(requestBody.password, oldManager.password);
            }catch(e){
                //no op
            }
            if(compareToMatch){
        ...
        if(requestBody.manager_level && requestBody.manager_level !== oldManager.manager_level){
        ...
        res.status(404).json({error: `manager with id ${req.params.id} not found!`});
        return;
    }
...
router.patch('/:mId/:userId/:postId', async (req,res) =>{
    ...
    if(!req.params.userId){
        res.status(400).json({ error: `You must Supply a user ID to delete user's post` });
        return;
    }
...
router.patch('/:mId/:userId/:commentId', async (req,res) =>{
    ...
    if(!req.params.userId){
        res.status(400).json({ error: `You must Supply a user ID to delete user's comment` });
        return;
    }
